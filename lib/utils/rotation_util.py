...
def rotate_points_torch(points, rys):
    """
    Rotate rys
    points: [..., n, 3]
    rys: [...]
    ...
    points = points.transpose(1, 2) # [..., 3, n]
    ones = torch.ones_like(c)
    zeros = torch.zeros_like(c)
    row1 = torch.stack([c,zeros,s], dim=-1) # [...,3]
    row2 = torch.stack([zeros,ones,zeros], dim=-1)
    row3 = torch.stack([-s,zeros,c], dim=-1)
    R = torch.stack([row1, row2, row3], dim=-2) # (...,3,3)
    canonical_points = torch.matmul(R, points) # [..., 3, n]
    canonical_points = canonical_points.transpose(1, 2) # [b, n, 3]
    return canonical_points
