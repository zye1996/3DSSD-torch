...
def encode_angle2class_torch(angle, num_class):
    angle = angle - torch.floor(torch.div(angle, (2 * np.pi))) * 2 * np.pi
    assert angle.min() >= 0

    angle_per_class = 2*np.pi/float(num_class)
    shifted_angle = torch.fmod(angle+angle_per_class/2, 2*np.pi)
    class_id_f = torch.floor(torch.div(shifted_angle, angle_per_class))
    class_id = class_id_f.long()
...
def encode_dist_anchor_free(gt_ctr, gt_offset, anchor_ctr, anchor_offset=None):
    ...
    padding_translate = torch.stack([padding_zeros, padding_half_height, padding_zeros], dim=-1)  # [bs, points_num, 3]
