...
def cal_ciou_3d(box3d1:torch.Tensor, box3d2:torch.Tensor, enclosing_type:str="smallest"):
    """calculated 3d DIoU loss. assume the 3d bounding boxes are only rotated around z axis
    ...
    Returns:
     ...
     (torch.Tensor): (B, N) 3d IoU
    """
    diou, iou3d = cal_diou_3d(box3d1, box3d2, enclosing_type)
    v = 4 / math.pi * (torch.arctan(box3d1[..., 3]) / torch.arctan(box3d1[..., 4]) -
                       torch.arctan(box3d2[..., 3]) / torch.arctan(box3d2[..., 4])).square()
    alpha = v / (1 - iou3d + v + 1e-8)
    ciou = diou - alpha * v
    return ciou, iou3d
