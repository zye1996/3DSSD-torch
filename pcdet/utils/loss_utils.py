...
class QualityFocalLoss(nn.Module):
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor):
        ...
        loss = self.sigmoid_cross_entropy_with_logits(input, zerolabel)

        # FG cat_id: [0, num_classes -1], BG cat_id: num_classes
        bg_class_ind = input.size(1)
        label = label - 1
        ...
        loss[pos, pos_label] = self.sigmoid_cross_entropy_with_logits(
            input[pos, pos_label], score[pos]) * scale_factor.abs().pow(self.gamma)
